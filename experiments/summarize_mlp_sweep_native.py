...
modes=sys.argv[1:] or ['exact','fc2']
files=['masks.u8.bin','scores.f32.bin','boxes.f32.bin','queries.i64.bin']
report={'base':'ba018a0','mlp':'int8_boundary','projection':'qkv','attention':'exact','qkv_rope':'fused',
...
for mode in modes:
    ...
    for i in [1,2]:
        ...
        reference=Path('.cache/native-perf/qkv-rope-ba018a0/timing-fused-truck-1')
...
for case in ['truck','bag','child','wheel','empty']:
    ref=Path(f'.cache/native-perf/qkv-rope-ba018a0/quality-fused-{case}-1')
    for mode in modes:
...
Path('experiments/results/native_rtx2060/mlp-sweep-ba018a0.json').write_text(json.dumps(report,indent=2)+'\n')
