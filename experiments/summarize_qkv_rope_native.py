...
report={'base':'ba018a0','mlp':'int8_boundary','projection':'qkv','attention':'exact',
...
for mode in ['exact','fused']:
    ...
    for i in [1,2]:
        ...
        reference=Path('.cache/native-perf/attention-ba018a0/timing-exact-truck-1')
...
for case in ['truck','bag','child','wheel','empty']:
    ref=Path(f'.cache/native-perf/attention-ba018a0/quality-exact-{case}-1')
    for mode in ['exact','fused']:
...
Path('experiments/results/native_rtx2060/qkv-rope-ba018a0.json').write_text(json.dumps(report,indent=2)+'\n')
