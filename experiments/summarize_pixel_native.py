...
for mode in ['exact','borrow','borrow_relu','nchw','fused_nchw']:
    for case in ['truck','bag','child','wheel','empty']:
        p=root/f'quality-{mode}-{case}-1'
        if not (p/'metrics.json').exists():continue
        mb,b,sb,bb=load(p)
        for label,base in [('exact',root/f'quality-exact-{case}-1'),('fp16',Path('.cache/native-perf/boundary-ba018a0')/f'quality-exact-{case}-1')]:
        ...
        if mode=='exact':
            old=Path('.cache/native-perf/kitchen-layout-ba018a0')/f'quality-sequence-{case}-1'
...
Path('experiments/results/native_rtx2060/pixel-ba018a0.json').write_text(json.dumps(report,indent=2)+'\n')
