"""Compare external ComfyKitchen attention against the existing INT8 model."""
import json
import os
...
root=Path('.cache/native-perf/kitchen-ba018a0')
...
kind=sys.argv[1]
modes=sys.argv[2:] or ['exact','kitchen','kitchen_rot','kitchen_all','kitchen_rot_all']
...
for mode,case,warmups,repeats in jobs:
    ...
    for k in ['SAM3_PROFILE_NVTX','SAM3_PROFILE_CAPTURE','SAM3_BENCH_CUDNN','SAM3_FUSED_NORM_CAST']:
        env.pop(k,None)
    env.update(SAM3_EXPERIMENT_MLP='int8_boundary',SAM3_EXPERIMENT_PROJECTION='qkv',SAM3_EXPERIMENT_ATTENTION=mode,SAM3_EXPERIMENT_QKV_ROPE='fused',SAM3_EXPERIMENT_BOUNDARY='exact',SAM3_EXPERIMENT_FC2_NORM='fused',SAM3_EXPERIMENT_INT8_LT='exact',SAM3_EXPERIMENT_INT4_FC2='exact',SAM3_EXPERIMENT_RESTORE='flat',TORCH_BLAS_PREFER_CUBLASLT='0')
    env['SAM3_EXPERIMENT_KITCHEN_LAYOUT']='head'
    env.pop('SAM3_EXPERIMENT_MLP_SCOPE',None)
    env.pop('SAM3_EXPERIMENT_PIXEL',None)
    if mode=='global_mlp':
        env.update(SAM3_EXPERIMENT_ATTENTION='kitchen_all',SAM3_EXPERIMENT_MLP_SCOPE='global',SAM3_EXPERIMENT_KITCHEN_LAYOUT='sequence')
    if mode=='attention_only':
        for key in list(env):
            if key.startswith('SAM3_EXPERIMENT_'):env.pop(key)
