"""Compare upstream FP16, original INT8, and fused-boundary INT8 on the same build."""
import json
import os
...
kind=sys.argv[1]
if kind=='timing':
    jobs=[(m,'truck',5,15) for m in ['exact','int8','int8_boundary','int8_boundary','int8','exact']]
elif kind=='quality':
    jobs=[(m,c,0,1) for c in cases for m in ['exact','int8','int8_boundary']]
...
for mode,case,warmups,repeats in jobs:
    ...
    for k in ['SAM3_PROFILE_NVTX','SAM3_PROFILE_CAPTURE','SAM3_BENCH_CUDNN','SAM3_FUSED_NORM_CAST','SAM3_EXPERIMENT_PROJECTION','SAM3_EXPERIMENT_RESTORE','SAM3_EXPERIMENT_ATTENTION']:
        env.pop(k,None)
    env.update(SAM3_EXPERIMENT_MLP=mode,TORCH_BLAS_PREFER_CUBLASLT='0')
