"""Attention projection timing and quality versus both previous INT8 and FP16."""
import json
...
for mode in ['exact','qkv','proj','both']:
    runs=[]
    for p in sorted(root.glob(f'timing-{mode}-truck-*')):
        ...
        if mode=='exact':
            assert all((p/f).read_bytes()==(previous/'timing-int8_boundary-truck-1'/f).read_bytes() for f in files)
...
for case in ['truck','bag','child','wheel','empty']:
    for mode in ['exact','qkv','proj','both']:
        ...
        b,sb,bb=load(p)
        for reference,base in [('fp16',previous/f'quality-exact-{case}-1'),('previous',previous/f'quality-int8_boundary-{case}-1')]:
            a,sa,ba=load(base)
...
dest=Path('experiments/results/native_rtx2060/projection-ba018a0.json')
