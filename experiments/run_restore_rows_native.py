"""Compare row-indexed INT8 restoration, with and without QKV quantization."""
import json
import os
...
kind=sys.argv[1]
if kind=='timing':
    jobs=[(m,'truck',5,15) for m in ['base','rows','qkv','qkv_rows','qkv_rows','qkv','rows','base']]
elif kind=='quality':
    jobs=[(m,c,0,1) for c in cases for m in ['rows','qkv_rows']]
...
for mode,case,warmups,repeats in jobs:
    ...
    for k in ['SAM3_PROFILE_NVTX','SAM3_PROFILE_CAPTURE','SAM3_BENCH_CUDNN','SAM3_FUSED_NORM_CAST']:
        env.pop(k,None)
    env.update(SAM3_EXPERIMENT_MLP='int8_boundary',SAM3_EXPERIMENT_PROJECTION='qkv' if mode.startswith('qkv') else 'exact',SAM3_EXPERIMENT_ATTENTION='exact',TORCH_BLAS_PREFER_CUBLASLT='0')
    env['SAM3_EXPERIMENT_RESTORE']='rows' if 'rows' in mode else 'flat'
