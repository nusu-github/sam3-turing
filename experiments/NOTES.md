...
## Round 65の候補：大きい入力成分をFP16へ分離

Sciteで検索した[LLM.int8](https://arxiv.org/abs/2208.07339)の混合精度分解を参考に、
QKV入力の一部だけをFP16の小さいLinearで計算し、残りを既存の動的INT8へ渡す。
評価画像とは別の動画先頭フレーム1枚から最大512token/層を取り、絶対値の大きい8/16/32成分を選ぶ。
全層またはglobal層だけに適用する。重みは既存のINT8からFP16へ復元したものなので、
原論文の再現でも、元のFP16重みの回復でもない。入力の量子化誤差を減らす効果と追加計算の負担を見る。
CPUテキストpadding省略・重み調整・非対称MLP・画像追加パッチは維持する。

## CPUテキストpadding省略後のスレッド数候補

`cpu_text_threads.py`でFP32とMLP動的INT8を1/2/4/8/16スレッドで比較する。
短い1文、長い1文、4 captionの3条件。GPUへの転送・画像推論を除いたCPU部分だけの簡単な測定で、
画像全体の速度としては扱わない。速い設定だけを後で画像5条件でも測る。
公開パッチがグローバルなスレッド数を変更する予定はない。

## Round 64：980 / 952 / 924入力

| 構成 | ms | allocated GiB | NVML GiB | IoU vs stock | 変化画素 |
|---|---:|---:|---:|---:|---:|
| r64_refined1008_control | 88.683 | 0.7816 | 2.3174 | 0.99799737 | 674 |
| refined_resolution980 | 89.837 | 0.7706 | 2.6839 | 0.97345661 | 7658 |
| refined_resolution952 | 89.911 | 0.7575 | 2.3584 | 0.97624083 | 8306 |
| refined_resolution924 | 96.93 | 0.7469 | 2.3643 | 0.97251135 | 8409 |

全構成1/4/6/4/0。今回の小幅な解像度低下では速度改善がなく、マスク差も増えた。
allocatedは0.782→0.747GiBまで減ったが、既定の1008入力を維持する。
CPUテキストを毎回計算する4スレッド構成の値。GPU単独の演算速度をこの値から断定しない。

## CPUテキスト単体のスレッド比較

[測定値](results/cpu_text_trimmed_thread_probe.json)。短文 / 長文 / 4 caption、padding省略後。
FP32の4スレッドは60.24 / 107.03 / 114.79ms、16スレッドは42.44 / 89.05 / 88.28ms。
CPU INT8の4スレッドは35.11 / 58.79 / 69.84ms、8スレッドは30.23 / 47.71 / 70.15ms。
全出力finiteでテキストmaskは一致。FP32のthread変更だけなら今回のtext memory差は0。
INT8ではFP32からtext memoryの最大差が短文0.462、長文3.424、batch4で0.684。画像マスクの差ではない。

## Round 66の候補：低解像度時のCPU待ちを減らす

GPU画像処理は非同期なので、CPUテキスト単体の時間を全体時間に単純加算できない。
