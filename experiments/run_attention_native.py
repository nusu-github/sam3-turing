"""Compare external Turing attention forward on the existing INT8 MLP/QKV baseline."""
import json
import os
...
kind=sys.argv[1]
if kind=='timing':
    jobs=[(m,'truck',5,15) for m in ['exact','global32','global64','global64','global32','exact']]
elif kind=='quality':
    jobs=[(m,c,0,1) for c in cases for m in ['exact','global32','global64']]
...
for mode,case,warmups,repeats in jobs:
    ...
    for k in ['SAM3_PROFILE_NVTX','SAM3_PROFILE_CAPTURE','SAM3_BENCH_CUDNN','SAM3_FUSED_NORM_CAST','SAM3_EXPERIMENT_QKV_ROPE']:
        env.pop(k,None)
    env.update(SAM3_EXPERIMENT_MLP='int8_boundary',SAM3_EXPERIMENT_PROJECTION='qkv',SAM3_EXPERIMENT_ATTENTION=mode,SAM3_EXPERIMENT_RESTORE='flat',TORCH_BLAS_PREFER_CUBLASLT='0')
