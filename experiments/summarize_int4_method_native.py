"""Evaluate INT4 scope against fixed INT8 and FP16 references; never relax gates."""
import json
from pathlib import Path
import statistics as st
import numpy as np
from scipy.optimize import linear_sum_assignment
root=Path('.cache/native-perf/int4-method-ba018a0')
...
for mode in ['exact','w4','a4','affine','a4_affine','mse','a4_mse','w4_mse','rht16','rht64']:
    for case in ['truck','bag','child','wheel','empty']:
        p=root/f'quality-{mode}-{case}-1'
        if not (p/'metrics.json').exists():continue
        mb,b,sb,bb=load(p)
        for label,base in [('int8',root/f'quality-exact-{case}-1'),('fp16',Path('.cache/native-perf/boundary-ba018a0')/f'quality-exact-{case}-1')]:
        ...
        if mode=='exact':
            old=Path('.cache/native-perf/mlp-sweep-ba018a0')/f'quality-fc2-{case}-1'
...
Path('experiments/results/native_rtx2060/int4-method-ba018a0.json').write_text(json.dumps(report,indent=2)+'\n')
