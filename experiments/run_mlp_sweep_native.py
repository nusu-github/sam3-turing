"""Compare boundary scheduling/cache and FC2 restoration/norm variants."""
import json
import os
...
root=Path('.cache/native-perf/mlp-sweep-ba018a0')
...
kind=sys.argv[1]
modes=sys.argv[2:] or ['exact','fc2']
config={'exact':('exact','exact'),'fc2':('exact','fused'),'readonly':('readonly','exact'),'threads128':('threads128','exact'),'threads512':('threads512','exact'),'both':('readonly','fused')}
if kind=='timing':
...
for mode,case,warmups,repeats in jobs:
    ...
    for k in ['SAM3_PROFILE_NVTX','SAM3_PROFILE_CAPTURE','SAM3_BENCH_CUDNN','SAM3_FUSED_NORM_CAST']:
        env.pop(k,None)
    env.update(SAM3_EXPERIMENT_MLP='int8_boundary',SAM3_EXPERIMENT_PROJECTION='qkv',SAM3_EXPERIMENT_ATTENTION='exact',SAM3_EXPERIMENT_QKV_ROPE='fused',SAM3_EXPERIMENT_BOUNDARY=config[mode][0],SAM3_EXPERIMENT_FC2_NORM=config[mode][1],SAM3_EXPERIMENT_RESTORE='flat',TORCH_BLAS_PREFER_CUBLASLT='0')
