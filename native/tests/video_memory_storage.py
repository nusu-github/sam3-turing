...
def main():
    p=argparse.ArgumentParser();p.add_argument('library',type=Path);p.add_argument('store',type=Path);p.add_argument('checkpoint',type=Path);p.add_argument('--device',default='cuda');p.add_argument('--threads',type=int,default=4);p.add_argument('--modes',nargs='+',default=['fp16','fp32','bf16_reference']);p.add_argument('--report',type=Path,required=True);a=p.parse_args()
    torch.ops.load_library(str(a.library.resolve()));torch.set_num_threads(a.threads);torch.manual_seed(971)
    ...
    for mode in a.modes:
        ...
        # Check the live global rewrite before insertion rebuilds memory. A shared
        # channels-last image has a singleton batch stride that the source's
        # sequence-to-BCHW view changes; direct raw-image convolution can differ.
        state3=MultiplexController(16).eval().get_state(3,torch.device(a.device),torch.float32,random=False,object_ids=[101,202,303])
        with torch.autocast(a.device,enabled=mode!='fp32',dtype=dtype):
            memory3,position3=host._encode_new_memory(None,[arrays[0][4].flatten(2).permute(2,0,1)],[(72,72)],expected['2/cond/0/memory_masks'].to(a.device),expected['2/cond/0/memory_scores'].to(a.device),False,conditioning_objects={0,1,2},multiplex_state=state3)
        torch.testing.assert_close(expected['2/cond/0/memory'],memory3.to(torch.bfloat16).cpu(),rtol=0,atol=0);checks+=1
        torch.testing.assert_close(expected['2/cond/0/position'],position3[-1].cpu(),rtol=0,atol=0);checks+=1
    ...
    a.report.write_text(json.dumps(dict(cases=rows,device=a.device,threads=a.threads,scope='Native global memory write -> object insertion/history rebuild -> forward/reverse propagation -> correction/preflight, resident/offloaded/paged equivalence, plus actual source encoder comparisons immediately after global rewrite and after bucket rebuild, using channels-last shared image features. Predicted low masks/logits unchanged by memory rewrite; obsolete overrides cleared on preflight.'),indent=2)+'\n')
