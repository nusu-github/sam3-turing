...
def main():
    ...
    precision=dict(matmul_tf32=bool(torch.backends.cuda.matmul.allow_tf32),cudnn_tf32=bool(torch.backends.cudnn.allow_tf32))
    ...
    exact=all(all(r['exact'].values()) for r in rows);a.report.write_text(json.dumps(dict(model=a.model,mode='bf16_reference',source_grounding_batch=1,source_complex_rope=True,box_steps=a.box_steps,source_inclusive_grounding_bound_adapter=a.sam31_inclusive_grounding_bound,effective_precision=precision,source_preserve_batched_geometry_adapter=a.sam31_preserve_batched_geometry,source_geometry_adapter_uses=geometry_uses,cases=rows,exact=exact,scope='Original high-level semantic add_prompt, full propagation with output scheduling, text/box-only/combined replacement, fetch and reset versus owning C++ API. No neural phases mocked. Optional SAM3.1 forward grounding-bound and stored-geometry adapters are recorded separately; the source generator range/output scheduling is unchanged. Fixed sequence is a fixture, not API input limits. Native additionally checks invalid-input state preservation, point-only cache initialization and callback cancellation; those extra native invariants are not compared here.'),indent=2)+'\n')
