...
VideoMaskOutput VideoInteractiveHeads::forward(const at::Tensor& image,const std::vector<at::Tensor>& high,
    const at::Tensor& points,const at::Tensor& labels,const at::Tensor& masks,bool multimask,const std::string& mode,double object_threshold) const {
  ...
  if (multimask) {
    const auto best=output.iou.argmax(-1),ids=at::arange(batch,best.options());
  ...
  return {low,full,output.iou,selected_low,selected_full,pointer,output.object_logits};
}
