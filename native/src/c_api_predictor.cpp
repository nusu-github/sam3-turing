...
extern "C" {
sam3_status sam3_predictor_set_tracking_devices(sam3_predictor* handle,const char* const* devices,int64_t count)noexcept{return protect([&]{require(devices && count>0 && uint64_t(count)<=SIZE_MAX/sizeof(at::Device),"nonempty tracking device list required");std::vector<at::Device> selected;for(int64_t i=0;i<count;++i){require(devices[i],"null tracking device");selected.emplace_back(devices[i]);}predictor(handle,[&](auto& p){p.set_tracking_devices(selected);});});}
...
sam3_status sam3_predictor_set_parallel_tracking(sam3_predictor* handle,int32_t enabled)noexcept{return protect([&]{require(enabled==0 || enabled==1,"parallel tracking must be 0 or 1");predictor(handle,[&](auto& p){p.set_parallel_tracking(enabled!=0);});});}
