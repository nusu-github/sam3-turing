...
sam3_status sam3_video_create(sam3_context* context,const sam3_video_options* o,sam3_video** out) noexcept{return protect([&]{
  output(out);require(context,"context is required");options(o);require(o->frames>0 && o->height>0 && o->width>0 && o->provider,"video dimensions and frame provider are required");
  require(o->fill_hole_area>=0 && o->multimask_min_points>=0 && o->multimask_max_points>=o->multimask_min_points && std::isfinite(o->object_threshold),"invalid mask options");const auto timing=temporal(*o);
  ...
  const auto pixels=[provider,user,h,w](int64_t index){sam3_rgb_view view{};if(provider(user,index,&view)!=0)throw Failure(SAM3_CALLBACK_ERROR,"frame provider reported failure");require(view.height==h && view.width==w,"frame provider changed video dimensions");return rgb(view);};
  if(c.model=="sam3"){
